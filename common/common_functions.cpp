                         ...
                         bool printDeviceInfo)
{
    ...
    assert(err == CL_SUCCESS);
    int dev_type = (device_type == "gpu") ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU;
    for (int i = 0; i < ret_num_platforms; ++i) {
        err = clGetDeviceIDs(platforms[i], dev_type, 1, &device_id, &ret_num_devices);
        if (err == CL_SUCCESS)
            break;
    }
    if (err != CL_SUCCESS) {
...
std::string measureExecTime(Executor exec, std::string device_type, unsigned int repeat)
{
    double cpuTime = 0;
    double kernelTime = 0;
    for (int i = 0; i < repeat; ++i) {
        auto time = exec(device_type);
...
{
    std::string helpMsg = "Usage: " + std::string(argv[0]) + " [arg]\n\n" +
        "Available arguments:\n" +
    ...
    if (argc == 1) {
        ...
        if (arg1 == "cpu" || arg1 == "gpu") {
            err = 0;
            return argv[1];
        }
        std::string msg;
        if (arg1 != "h" && arg1 != "help") {
            msg = "Error! Invalid argument: \"" + std::string(argv[1]) + "\"!\n\n";
        }
        err = 1;
        msg += helpMsg;
        return msg;
